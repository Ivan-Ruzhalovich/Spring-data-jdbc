...
public class BookRepositoryImpl implements BookRepository{
    ...
    public void updateInfoAboutBook(Book updateForBook) {
        try{
            template.queryForObject(getBookById,new BookMapper(),updateForBook.getId());
            template.update(updateInfoAboutBook,updateForBook.getTitle(),updateForBook.getAuthor()
    ...
    public void deleteBook(Long id) {
        try{
            template.queryForObject(getBookById,new BookMapper(),id);
            template.update(deleteBookById,id);
        }catch (EmptyResultDataAccessException e){
            throw new ResponseStatusException(HttpStatus.NOT_FOUND,"Книна не найдена!");
        }
    }
